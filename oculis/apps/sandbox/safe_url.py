...
def _parse_literal_or_encoded_ip(hostname: str) -> ipaddress.IPv4Address | ipaddress.IPv6Address | None:
    try:
...
class _PinnedNetworkBackend(httpcore.AnyIOBackend):
    """HTTPCore backend that pins validated hostnames to specific IPs."""

    def __init__(self, mapping: dict[str, str]) -> None:
        super().__init__()
        self._mapping = {
            host.lower().rstrip("."): ip
    ...
    async def connect_tcp(
        ...
        pinned_ip = self._mapping.get(key)

        if pinned_ip is None:
            raise httpcore.ConnectError(
                f"no validated IP pin exists for {host}"
            )
...
def _pinned_transport(hostname: str, ip: str) -> httpx.AsyncHTTPTransport:
    """Create an HTTP transport that connects to the validated IP."""
    transport = httpx.AsyncHTTPTransport(
        retries=0,
        trust_env=False,
    )

    # httpx does not expose httpcore's network backend publicly.
    # Replacing the backend on its connection pool preserves the original
    # hostname for HTTP authority/TLS SNI while pinning the TCP destination.
    pool = transport._pool  # type: ignore[attr-defined]
    pool._network_backend = _PinnedNetworkBackend(  # type: ignore[attr-defined]
        {hostname: ip}
    )
